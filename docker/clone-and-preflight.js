...
(async () => {
    ...
    async function executeCommand(command, cwd = repoPath) {
    ...
    await executeCommand(`git clone --depth 1 --single-branch --branch=main ${process.argv[2]} ${repoPath} --config core.autocrlf=input`, process.cwd());
    await executeCommand('yarn install --ignore-scripts');
    await executeCommand('yarn upgrade --latest @upleveled/eslint-config-upleveled');
...
//# sourceMappingURL=clone-and-preflight.js.map