  ...
  if (stdout !== '') {
    throw new Error(
      `package-lock.json or yarn.lock file committed to Git. Remove it with:
        ...
        The presence of this file indicates that an incorrect package manager was used in addition to pnpm (eg. an "npm install" or a "yarn add" command was run). In order to avoid issues with the state of the pnpm-lock.yaml file, we suggest also forcing regeneration this file with the following command:
